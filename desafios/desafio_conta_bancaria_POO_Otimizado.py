...
class Conta(ContaCorrente):
    ...
    def sacar(self, valor):
        ...
        if self._saldo >= valor:
            transacoes_hoje = {
                data: transacao
                for data, transacao in self._historico.transacoes.items()
            ...
            if len(transacoes_hoje) < self._limite_transacoes:
                saques_hoje = {
                    data: transacao
                    for data, transacao in self._historico.transacoes.items()
                    if (data.date() == dt.date.today()) and type(transacao) == Saque
                }
    ...
    def criar_cliente(clientes):
        ...
        try:
            ...
            if cpf not in clientes.keys():
                ...
                clientes[cpf] = Cliente.criar_cliente(
        ...
        try:
            ...
            if cpf in clientes.keys():
                try:
                    ...
                    conta_do_usuario = next(
                        conta
                        for conta in contas_do_cpf
                    ...
                    try:
                        ...
                        if deposito.registrar(conta_do_usuario):
                            ...
                            print(
                                "Não foi possível concluir a transação, por gentileza validar o extrato"
                            )
                            print(
                                "É permitido realizar apenas 10 transações por dia, 3 saques diários no valor de R$500.00"
                            )
                    except Exception:
                ...
                try:
                    ...
                    try:
                        ...
                        if saque.registrar(conta_do_usuario):
                            ...
                            print(
                                "Não foi possível concluir a transação, por",
                                "gentileza validar o extrato",
                            )
                            print(
                                "É permitido realizar apenas 10 transações",
